...
trait CompanyCallsTrait
{
     ...
     * @throws NmbrsException
     */
    public function getAllRunsByYear(int $companyId, int $year): array
    {
        try {
    ...
    /**
     * getCurrentAddressByCompanyId
     * https://api.nmbrs.nl/soap/v3/CompanyService.asmx?op=Company_GetCurrentPeriod
     *
     * @param $id
     *
     * @return ?string
     ...
     * @throws NmbrsException
     */
    public function getCurrentPeriodByCompanyId(int $id): ?string
    {
        try {
            $response = $this->companyClient->Company_GetCurrentPeriod(['CompanyId' => $id]);

            return $response->Company_GetCurrentPeriodResult ?? null;
        } catch (\Exception $e) {
            throw new NmbrsException($e->getMessage());
        }
    }
